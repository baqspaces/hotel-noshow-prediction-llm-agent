...
The "Operational Queue" section of the web app dashboard uses the Random Forest ML no-show probability generated by the ipynb. The ipynb generates 5 other ML models and saved the corresponding model scores into a separate db called `booking_ml_scores.db`. This db contains a column each for each model score.
